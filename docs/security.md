...
| Mail credential exposure | There is no mail credential. The agreement is sent through the operator's own Google account with the `gmail.send` scope, granted per session and held in memory only; it cannot read a mailbox |
...
- The agreement is sent from the browser, so the delivery receipt carries no more authority than any other staff write: an approved account could create one directly. The receipt is evidence of a send, not a control over it. It is append-only under the rules, so a record cannot be edited or removed once written, and what actually reached a renter is also in the sender's own Gmail Sent folder, outside this application entirely.
